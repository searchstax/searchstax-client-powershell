# Script for listing the properties of a specific heartbeat alert. 
...
Write-Host "Getting heartbeat alert $ALERTNO from $uid"
# GET /api/rest/v2/account/{account_name}/deployment/{uid}/alerts/heartbeat/{id}/

$RESULTS = Invoke-RestMethod -uri "https://app.searchstax.com/api/rest/v2/account/$ACCOUNT/deployment/$uid/alerts/heartbeat/$ALERTID/" -Method Get -Headers $headers
