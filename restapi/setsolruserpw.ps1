# PowerShell script for setting a Solr Basic Auth user's password.
...
Write-Host "Changing Solr user's password..."
Write-Host $body

#POST https://app.searchstax.com/api/rest/v2/account/<account_name>/deployment/<uid>/solr/auth/set-password/
$RESULT = Invoke-RestMethod -uri "https://app.searchstax.com/api/rest/v2/account/$ACCOUNT/deployment/$uid/solr/auth/set-password/" -Method Post -body $body -ContentType 'application/json' -Headers $headers
