...
# users > create
# Script for creating a user in a SearchStax account. 
...
Write-Host "Adding a user to $ACCOUNT"
Write-Host
...
# POST /api/rest/v2/account/?account={account_name}&searchstudio={true/false}

$RESULTS = Invoke-RestMethod -Method Post -Body $body -ContentType 'application/json' -Headers $headers `
          -uri "https://app.searchstax.com/api/rest/users/?account=$ACCOUNT&searchstudio=false" 
